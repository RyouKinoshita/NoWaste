...
const ProcessCart = () => {
    ...
    const updateQuantity = (prodID, newQuantity, action) => {
        ...
        setTax(setTaxPrice)
        setFullPrice(fullPrice + (updatedFullPrice - (prod.price * cartItem.quantity)));
    };
    ...
    const handleSubmit = async (e) => {
        try {
            ...
            const { data } = await axios.post(`${process.env.REACT_APP_API}order-create`, orderData, config);
            ...
            <Container className='my-5'>
                <Row>
                    <Card>
                        ...
                        <MDBBtn style={{ backgroundColor: 'orange' }} onClick={handleToggleBin}><p>Bin</p><i className="fa-solid fa-trash"></i></MDBBtn>

                        <MDBBtn style={{ backgroundColor: 'orange' }} onClick={handleToggleCart}><p>Cart</p><i class="fa-brands fa-opencart"></i></MDBBtn>

                        <Card className='my-3'>
                            <Row className="px-4">
                                <form ref={formRef} onSubmit={handleSubmit} className="shadow-lg" style={{ border: "solid 4px white" }}>
                                    {/* Render cart items */}
                                    {cartItems
                                        .filter(item => showOutCart ? item.status === 'outCart' : item.status === 'onCart') // Filter out items based on status
                                        .map(item => (
                                                                        ...
                                                                        <div className="px-3 my-2">
                                                                            <Row>
                                                                                <Col style={{ width: '40px' }}>
                                                                                    ...
                                                                                    <button type="button" onClick={() => updateQuantity(item.prodID, item.quantity - 1, 'decrease')}>-</button>
                                                                                    <span>Quantity: {item.quantity}</span>
                                                                                    <button type="button" onClick={() => updateQuantity(item.prodID, item.quantity + 1, 'increase')} disabled={product.sack === 0}>+</button>
                                                                                    <p></p>
                                                                                ...
                                                                                {showOutCart && (
                                                                                    <>
                                                                                        ...
                                                                                        <Col>
                                                                                            <MDBBtn style={{ backgroundColor: 'orange' }} onClick={() => handleonCart(item._id)}><i class="fa-solid fa-repeat"></i></MDBBtn>
                                                                                        </Col>
                                                                                    </>
                                                                                )}
                                                                                {!showOutCart && (
                                                                                    <>
                                                                                        ...
                                                                                        <Col style={{ marginRight: '100px' }}>
                                                                                            <MDBBtn className="btn btn-danger" onClick={() => handleoutCart(item._id)}><i class="fa-solid fa-trash"></i></MDBBtn>
                                                                                        </Col>
                                                                                    </>
                                                                                )}
                                                                            </Row>
                                    ...
                                    {!showOutCart && (
                                        <>
                                            ...
                                            <Card className="my-3">
                                                <MDBInput className="my-2" label='Set Address' type='text' name={'shippingInfo-address'} value={address} onChange={(e) => setAddress(e.target.value)} />
                                                <MDBInput className="my-2" label='Set City' type='text' name={'shippingInfo-city'} value={city} onChange={(e) => setCity(e.target.value)} />
                                                <MDBInput className="my-2" label='Phone No.#' type='text' name={'shippingInfo-phoneNo'} value={phoneNo} onChange={(e) => setPhoneNo(e.target.value)} />
                                                <MDBInput className="my-2" label='Set Postal Code' type='text' name={'shippingInfo-postalCode'} value={postalCode} onChange={(e) => setPostalCode(e.target.value)} />
                                                <MDBInput className="my-2" label='Set Country' type='text' name={'shippingInfo-country'} value={country} onChange={(e) => setCountry(e.target.value)} />
                                                <div className="my-2">
                                            ...
                                            <div value={userId} />
                                            <MDBBtn type="submit" color='light' rippleColor='dark'>Checkout</MDBBtn>
                                        </>
                                    )}
                                </form>
